...
Yarn 安装依赖速度还是慢的话，可以用 [tyarn](https://www.npmjs.com/package/tyarn)。
...
* menu 和 router 合成一个。
* 添加 Blank layout。
* 项目内的代码生成。 参考 [umi](https://umijs.org/zh-CN)。
  * 创建页面。 配置页面类型(列表，详情等), 路由地址。
  * 页面内生成代码。 最好能解析 AST，否则，只能是一个弹出框，显示加的代码。
    * 列表页： 添加搜索条件，添加列(列名，类型:最后可以读数据库的那张表；)
    * 更新页： 添加字段。
